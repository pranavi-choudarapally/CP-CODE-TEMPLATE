...
class Main{
    ...
    static class Bit {
        ...
        // Purpose: Convert Decimal to Binary String
        // Input: int number
        // Output: String
        public static String decToBin(int n) {
            String res = Integer.toBinaryString(n);
            DBG.log("Bit.decToBin (" + n + ")", res);
            return res;
        }

        // Purpose: Set k-th Bit (0-based indexing)
        // Input: int number, int bitPosition
        // Output: int
        public static int setBit(int n, int k) {
            int res = n | (1 << k);
            DBG.log("Bit.setBit (" + n + "," + k + ")", res);
            return res;
        }

        // Purpose: Get k-th Bit (0 or 1)
        // Input: int number, int bitPosition
        // Output: int
        public static int getBit(int n, int k) {
            int res = (n >> k) & 1;
            DBG.log("Bit.getBit (" + n + "," + k + ")", res);
            return res;
        }

        // Purpose: Toggle k-th Bit
        // Input: int number, int bitPosition
        // Output: int
        public static int toggleBit(int n, int k) {
            int res = n ^ (1 << k);
            DBG.log("Bit.toggleBit (" + n + "," + k + ")", res);
            return res;
        }

        // Purpose: Clear k-th Bit
        // Input: int number, int bitPosition
        // Output: int
        public static int clearBit(int n, int k) {
            int res = n & ~(1 << k);
            DBG.log("Bit.clearBit (" + n + "," + k + ")", res);
            return res;
        }
    }
...
public static void main(String[] args) {
        ...
        // CALL SYNTAX: Bit Class
        // String binary = Bit.decToBin(n);
        // int x = Bit.setBit(n, k);
        // int bit = Bit.getBit(n, k);
        // int y = Bit.toggleBit(n, k);
        // int z = Bit.clearBit(n, k);
